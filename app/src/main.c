...
#define WDG_ALL_TASKS_BITS                                                     \
  (WDG_BIT_SENSOR_READER | WDG_BIT_PROCESSOR | WDG_BIT_OUTPUT |                \
   WDG_BIT_HEARTBEAT)
...
uint16_t merge(uint8_t low, uint8_t high) { return (low | (high << 8)); }
...
void vSensorProcessorTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    ProcessedData xProcessedData = {
        {xData.accel_x / 16384.0f, xData.accel_y / 16384.0f,
         xData.accel_z / 16384.0f},
        {xData.gyro_x / 131.0f, xData.gyro_y / 131.0f, xData.gyro_z / 131.0f}};

    if (xProcessorQueue != 0) {
      if (xQueueSend(xProcessorQueue, (void *)&xProcessedData,
                     pdMS_TO_TICKS(10))) {
      } else {
...
void vSensorOutputTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    printf("Accel_x: %f, Accel_y: %f, Accel_z: %f, Gyro_x: %f, Gyro_y: %f, "
           "Gyro_z: %f\r\n",
           xData.accel_g[0], xData.accel_g[1], xData.accel_g[2],
           xData.gyro_dps[0], xData.gyro_dps[1], xData.gyro_dps[2]);
  }
...
void vWatchdogTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    EventBits_t bits_before_reset =
        xEventGroupClearBits(xWatchdogEvents, WDG_ALL_TASKS_BITS);
...
int main(void) {
  ...
  xReturned =
      xTaskCreate(vSensorReaderTask, "SensorReader", configMINIMAL_STACK_SIZE,
                  NULL, tskIDLE_PRIORITY + 4, &xTaskHandle1);
  ...
  xReturned = xTaskCreate(vSensorProcessorTask, "SensorProcessor",
  ...
  xReturned = xTaskCreate(vSensorOutputTask, "SensorOutput",
  ...
  xReturned =
      xTaskCreate(vHeartBeatTask, "HeartBeat", configMINIMAL_STACK_SIZE + 64,
                  NULL, tskIDLE_PRIORITY + 2, &xTaskHandle4);
  ...
  xReturned = xTaskCreate(vButtonTask, "Button", configMINIMAL_STACK_SIZE * 4,
                          NULL, tskIDLE_PRIORITY + 1, &xTaskHandle5);
  ...
  xReturned =
      xTaskCreate(vWatchdogTask, "Watchdog", configMINIMAL_STACK_SIZE * 4, NULL,
