...
#define STAI_NETWORK_ORIGIN_MODEL_NAME      "ds_cnn_s"
#define STAI_NETWORK_ORIGIN_MODEL_SIGNATURE "0x550d57c1752b06c128a6a340588e4067"

/*  Generated model name and signature  */
#define STAI_NETWORK_MODEL_NAME      "network"
#define STAI_NETWORK_MODEL_SIGNATURE (0xc0617306f5233384)
...
#define STAI_NETWORK_NODES_NUM (16)

/*  Macro to declare number macc for generated network model  */
#define STAI_NETWORK_MACC_NUM (2665536)

/*  Macros to declare input buffers characteristics  */
#define STAI_NETWORK_IN_NUM        (1)
#define STAI_NETWORK_IN_SIZE_BYTES (496)

#define STAI_NETWORK_IN_ALIGNMENTS  {4}
#define STAI_NETWORK_IN_NAMES       {NULL}
#define STAI_NETWORK_IN_FORMATS     {STAI_FORMAT_S8}
#define STAI_NETWORK_IN_SIZES       {490}
#define STAI_NETWORK_IN_SIZES_BYTES {490}

#define STAI_NETWORK_IN_1_ALIGNMENT 4
#define STAI_NETWORK_IN_1_NAME      (NULL)
#define STAI_NETWORK_IN_1_FLAGS                                                                    \
    (STAI_FLAG_INPUTS | STAI_FLAG_PREALLOCATED | STAI_FLAG_CHANNEL_LAST | STAI_FLAG_HAS_BATCH)
#define STAI_NETWORK_IN_1_FORMAT     (STAI_FORMAT_S8)
#define STAI_NETWORK_IN_1_SHAPE      {1, 49, 10, 1}
#define STAI_NETWORK_IN_1_BATCH      (1)
#define STAI_NETWORK_IN_1_HEIGHT     (49)
#define STAI_NETWORK_IN_1_WIDTH      (10)
#define STAI_NETWORK_IN_1_CHANNEL    (1)
#define STAI_NETWORK_IN_1_RANK       (3)
#define STAI_NETWORK_IN_1_SIZE       (490)
#define STAI_NETWORK_IN_1_SIZE_BYTES (490)
#define STAI_NETWORK_IN_1_SCALE      (0.6043851971626282f)
#define STAI_NETWORK_IN_1_ZERO_POINT (76)

/*****************************************************************************/
/*  Macros to declare output buffers characteristics  */
#define STAI_NETWORK_OUT_NUM         (1)
#define STAI_NETWORK_OUT_SIZE_BYTES  (16)
#define STAI_NETWORK_OUT_ALIGNMENTS  {4}
#define STAI_NETWORK_OUT_NAMES       {NULL}
#define STAI_NETWORK_OUT_FORMATS     {STAI_FORMAT_S8}
#define STAI_NETWORK_OUT_SIZES       {12}
#define STAI_NETWORK_OUT_SIZES_BYTES {12}

#define STAI_NETWORK_OUT_1_ALIGNMENT 4
#define STAI_NETWORK_OUT_1_NAME      (NULL)
#define STAI_NETWORK_OUT_1_FLAGS                                                                   \
    (STAI_FLAG_OUTPUTS | STAI_FLAG_PREALLOCATED | STAI_FLAG_CHANNEL_LAST | STAI_FLAG_HAS_BATCH)
#define STAI_NETWORK_OUT_1_FORMAT     (STAI_FORMAT_S8)
#define STAI_NETWORK_OUT_1_SHAPE      {1, 12}
#define STAI_NETWORK_OUT_1_BATCH      (1)
#define STAI_NETWORK_OUT_1_CHANNEL    (12)
#define STAI_NETWORK_OUT_1_RANK       (1)
#define STAI_NETWORK_OUT_1_SIZE       ((12))
...
#define STAI_NETWORK_ACTIVATIONS_SIZE       (18252)
#define STAI_NETWORK_ACTIVATIONS_SIZE_BYTES (18252)
#define STAI_NETWORK_ACTIVATIONS_ALIGNMENTS {4}
#define STAI_NETWORK_ACTIVATIONS_SIZES      {18252}
#define STAI_NETWORK_ACTIVATIONS_NUM        (1)

#define STAI_NETWORK_ACTIVATION_1_FLAGS      (STAI_FLAG_ACTIVATIONS)
#define STAI_NETWORK_ACTIVATION_1_SIZE       (18252)
#define STAI_NETWORK_ACTIVATION_1_SIZE_BYTES (18252)
...
#define STAI_NETWORK_WEIGHTS_SIZE        (24368)
#define STAI_NETWORK_WEIGHTS_SIZE_BYTES  (24368)
#define STAI_NETWORK_WEIGHTS_ALIGNMENTS  {4}
#define STAI_NETWORK_WEIGHTS_SIZES       {24368}
#define STAI_NETWORK_WEIGHTS_NUM         (1)
#define STAI_NETWORK_WEIGHT_1_FLAGS      ((STAI_FLAG_PREALLOCATED))
#define STAI_NETWORK_WEIGHT_1_SIZE       (24368)
#define STAI_NETWORK_WEIGHT_1_SIZE_BYTES (24368)
...
#define STAI_NETWORK_STATES_SIZES                                                                  \
    {                                                                                              \
        0,                                                                                         \
    }
 ...
 * @details The application must provide the memory area where to initialize the internal network
 * context as an opaque handler named network. Application must never access directly the directly
 * the context but it must be accessed only with associates stai APIs. Application knows network
 * context size by using macro
 ...
 * @return STAI_SUCCESS on correct initialization, else 1st error return code from @ref
 * stai_return_code enums.
 */
STAI_API_ENTRY
 ...
 * @details The application must provide the memory area where to initialize the internal network
 * context as an opaque handler named netwrok. Application must never access directly the directly
 * the context but it must be accessed only with associates stai APIs.
 * @param[in] network network context opaque pointer
 * @return STAI_SUCCESS on correct de-initialization, else 1st error return code from @ref
 * stai_return_code enums.
 */
STAI_API_ENTRY
 ...
 * @details This API runs a newtwork inference. Network input data is expected to be available in
 * expected format on the input buffers whose memory adresses are retrived using @ref
 * stai_network_get_inputs() API. Alternatively the application could allocate the inputs and set
 * the addresses using the
 *          @ref stai_network_set_inputs() API. Similar behaviour is expected for the network
 * outputs handling. The features of each input / output buffer (format, byte size, etc.)may be
 * queried using
 *          @ref stai_network_get_info() API or the set of specific macros available in this header
 * file. When mode is MODE_SYNC the call to this API is blocking untile the end of the inference. In
 * ASYNC_MODE the API will exit before the completion of the inference by returning the current
 * state of the inference. It could be also used to query wether or not the inference has been
 * completed. After completions of run API the results of the network inference may be retrieved at
 * addresses returned by
 *          @ref stai_network_get_outputs() API.
 * @param[in] network network context opaque pointer
 * @param[in] mode the modality used to run the network. It may be synchronous (MODE_SYNC) or
 * asyncronous (MODE_ASYNC). see @ref stai_run_mode enum
 ...
 * @details This API is used to return the size in byte of the network internal context. The st.ai
 * APIs could support multiple network contexts, thus tt is expected that the network context is
 * always allocated by application but MUST be handled / queried / managed only by using st.ai APIs
 ...
 * @param[in] network network context opaque pointer
 * @param[out] activations an array of pointers reporting the addresses of all the activation
 * buffers
 * @param[out] n_activations the number of the network activations (i.e. the size of the activations
 * array)
 ...
 * @param[in] network network context opaque pointer
 * @param[out] outputs an array of pointers reporting the addresses of all the network outputs
 * buffers
 ...
 * @details as an array of pointers of size n_states. Optional API for stateful newtorks. It gets
 * the addresses of persistent internal states of the network. This APIs allows an application to
 * manage stateful buffers (e.g. to save them). State buffers may be set back using
 *          @ref stai_network_set_states() dual API
 * @param[in] network network context opaque pointer
 * @param[out] states an array of pointers reporting the addresses of all the network internal
 * states buffers
 * @param[out] n_states the number of the network internal states (i.e. the size of the states
 * array)
 * @return STAI_SUCCESS if successfull, else error 1st return code from @ref stai_return_code enums.
 * @note outputs array pointers are passed by copy
 */
 ...
 * @details In particular it return STAI_SUCCESS if no errors encountered during APIs orchestration,
 * otherwise this API return the 1st error generated.
 * @param[in] network network context opaque pointer
 ...
 * @param[in] n_activations the number of the network activations (i.e. the size of the activations
 * array)
 * @return STAI_SUCCESS if successfull, else error 1st return code from @ref stai_return_code enums.
 * @note activations array pointers are passed by copy
 */
 ...
 * @param[in] network network context opaque pointer
 * @param[in] outputs an array of pointers reporting the addresses of all the network outputs
 * buffers
 ...
 * @param[in] network network context opaque pointer
 * @param[in] states an array of pointers reporting the addresses of all the network internal states
 * buffers
 ...
 * @details this is an API to retrieve the content on intermediate activations buffers while
 * executing the
 *          @ref stai_network_run() API. It is a helper routine that could be used by an application
 * for debug and / or benchmarking intermadiate results of the network.
 * @param[in] network network context opaque pointer
 * @param[in] cb a function pointer to the inspection callback routine. See @ref stai_event_cb
 * function pointer for a definition of the callback API.
